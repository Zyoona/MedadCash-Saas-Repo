...
const parties_service_js_1 = require("../parties/parties.service.js");
const shared_types_1 = require("@medad/shared-types");
...
let SalesService = class SalesService {
    ...
    inventory;
    parties;
    constructor(prisma, ledger, inventory, parties) {
        this.prisma = prisma;
        this.ledger = ledger;
    ...
    async assertBankAccount(tenantId, code) {
        if (code === LEGACY_BANK)
            return;
        ...
        if (!bank)
            throw new common_1.BadRequestException(`كود الحساب البنكي غير مسجل أو معطل: ${code}`);
    }
    async createInvoice(tenantId, actor, input) {
        const branchId = input.branchId;
        if (!input.lines?.length)
            throw new common_1.BadRequestException('لا توجد أسطر');
        if (!input.payments?.length)
            throw new common_1.BadRequestException('لا توجد صفقات دفع');
        const date = input.date ? new Date(input.date) : new Date();
        const fy = await (0, ctx_js_1.fiscalYearFor)(this.prisma, tenantId, date);
        const taxDefault = ((await (0, ctx_js_1.settingValue)(this.prisma, tenantId, 'tax_rate', branchId)) ?? { rateBps: 0 }).rateBps;
        ...
        for (const l of input.lines) {
            ...
            if (!productId && l.variantId) {
                ...
                if (!v)
                    throw new common_1.NotFoundException('المتغير غير موجود');
                productId = v.productId;
            }
            if (!productId)
                throw new common_1.BadRequestException(' productId أو variantId مطلوب لكل سطر');
            ...
            const unitPriceAgora = l.unitPriceAgora ?? (pb ? (0, money_util_js_1.decToAgora)(pb.price) : 0);
            if (!Number.isInteger(unitPriceAgora) || unitPriceAgora < 0)
            ...
            resolved.push({
                ...
                costAgora: isService ? 0 : pb ? (0, money_util_js_1.decToAgora)(pb.cost) : 0,
                isService,
            });
        }
        // Binding math from shared-types (unit-tested §9.1)
        const totals = (0, shared_types_1.computeInvoice)(resolved.map((r) => ({ qty: r.qty, unitPriceAgora: r.unitPriceAgora, lineDiscountAgora: r.lineDiscountAgora, taxRateBps: r.taxRateBps })), input.invoiceDiscountAgora ?? 0);
        ...
        if (remainder < 0)
            throw new common_1.BadRequestException(`المدفوع (${paidNet}) أكبر من الإجمالي (${totals.grandTotalAgora})`);
        for (const p of input.payments) {
            if (!Number.isInteger(p.amountAgora) || p.amountAgora === 0)
                throw new common_1.BadRequestException('مبلغ دفع غير صالح');
            if (p.method === 'bank')
                await this.assertBankAccount(tenantId, p.accountCode);
        }
        return this.prisma.$transaction(async (db) => {
            ...
            const warnings = [];
            if (remainder > 0) {
                if (customer.isCashDefault)
                    throw new common_1.BadRequestException('البيع الآجل يتطلب عميلاً حقيقياً (ليس الزبون النقدي)');
                const balance = await this.parties.customerBalance(tenantId, customer.id);
                const limit = (0, money_util_js_1.decToAgora)(customer.creditLimit);
                if (limit > 0 && balance + remainder > limit) {
                    const canOverride = actor.perms.includes('override_credit_limit');
                    if (!canOverride) {
                        throw new common_1.ForbiddenException(`تجاوز حد الدين: الرصيد ${balance / 100} + الجديد ${remainder / 100} > الحد ${limit / 100}`);
                    }
            ...
            for (const r of resolved) {
                ...
                if (res.negative)
                    warnings.push(`مخزون سالب: ${r.productId} → ${res.newQty}`);
            }
            // Invoice document
            const invoice = await db.invoice.create({
                data: {
                    ...
                    payments: {
                        create: input.payments.map((p) => ({ method: p.method, accountCode: p.accountCode, amount: (0, money_util_js_1.agoraToDec)(p.amountAgora) })),
                    },
                },
            });
            ...
            const saleLines = [
                ...[...netByAccount.entries()].flatMap(([code, amt]) => amt > 0
            ...
            await (0, ctx_js_1.syncOpTx)(db, { tenantId, branchId, entity: 'invoice', entityId: invoice.id, op: 'create', payload: { grandTotalAgora: totals.grandTotalAgora, customerId: customer.id } });
            return { invoiceId: invoice.id, totals, remainderAgora: remainder, cogsAgora: cogsTotal, warnings, saleEntryId, cogsEntryId };
        });
    }
    async listInvoices(tenantId, q) {
        const page = q.page ?? 1;
        ...
        return {
            ...
            rows: rows.map((i) => ({
                ...
                payments: i.payments.map((p) => ({ ...p, amountAgora: (0, money_util_js_1.decToAgora)(p.amount) })),
            })),
        };
    }
     ...
     */
    async createReturn(tenantId, actor, input) {
        const inv = await this.prisma.invoice.findFirst({
            where: { id: input.sourceInvoiceId, tenantId, deletedAt: null, status: 'posted' },
            include: { lines: true, customer: true },
        });
        if (!inv)
            throw new common_1.NotFoundException('الفاتورة الأصلية غير موجودة');
        if (!input.lines?.length)
            throw new common_1.BadRequestException('لا توجد أسطر إرجاع');
        const fee = input.restockingFeeAgora ?? 0;
        if (!Number.isInteger(fee) || fee < 0)
            throw new common_1.BadRequestException('خصم إرجاع غير صالح');
        if (input.refundMethod === 'bank')
            await this.assertBankAccount(tenantId, input.refundAccountCode);
        ...
        for (const rl of input.lines) {
            if (!Number.isInteger(rl.qty) || rl.qty <= 0)
                throw new common_1.BadRequestException('كمية غير صالحة');
            const orig = inv.lines.find((l) => (l.variantId ?? l.productId ?? '') === rl.variantId || l.variantId === rl.variantId);
            if (!orig)
                throw new common_1.BadRequestException(`السطر غير موجود في الفاتورة: ${rl.variantId}`);
            const returnedBefore = already.get(orig.variantId ?? orig.productId ?? '') ?? 0;
            if (returnedBefore + rl.qty > orig.qty)
                throw new common_1.BadRequestException('الكمية المرجعة تتجاوز المباع');
        ...
        if (refundGross < 0)
            throw new common_1.BadRequestException('خصم الإرجاع أكبر من قيمة المرتجع');
        return this.prisma.$transaction(async (db) => {
            // stock back in
            for (const it of items) {
                const productId = it.line.variantId
                    ...
                    : it.line.productId;
                if (!productId)
                    throw new common_1.BadRequestException('تعذر تحديد الصنف للمرتجع');
            ...
            await (0, ctx_js_1.auditTx)(db, { tenantId, actorId: actor.userId, branchId: inv.branchId, action: 'create', entity: 'returns', entityId: ret.id, diff: { sourceInvoiceId: inv.id, refundGross, fee, entryId }, ip: actor.ip ?? null, device: actor.device ?? null });
            await (0, ctx_js_1.syncOpTx)(db, { tenantId, branchId: inv.branchId, entity: 'sale_return', entityId: ret.id, op: 'create', payload: { sourceInvoiceId: inv.id, refundGross } });
    ...
    async listReturns(tenantId, branchId) {
...
exports.SalesService = SalesService = __decorate([
    ...
    __metadata("design:paramtypes", [prisma_service_js_1.PrismaService,
        ...
        parties_service_js_1.PartiesService])
], SalesService);
