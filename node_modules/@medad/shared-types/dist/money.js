...
export function toAgora(amount) {
...
export function fromAgora(agora) {
    const neg = agora < 0;
...
export function mulRate(agora, numerator, denominator) {
    return Math.round((agora * numerator) / denominator);
}
/** Display: "₪ 1,234.56" */
export function formatILS(agora) {
    const neg = agora < 0;
    const abs = Math.abs(Math.round(agora));
    const shekels = Math.floor(abs / AGORA_PER_SHEKEL);
...
export function assertIntegerAgora(value, field = 'amount') {
    if (!Number.isInteger(value))
        throw new Error(`Money must be integer agora: ${field}=${value}`);
}
